...
namespace Trinity
{
    ...
    {
        public:
            AnyFriendlyUnitInObjectRangeCheck(WorldObject const* obj, Unit const* funit, float range, bool playerOnly = false, bool incOwnRadius = true, bool incTargetRadius = true, bool noPlayerTarget = false)
                : i_obj(obj), i_funit(funit), i_range(range), i_playerOnly(playerOnly), i_incOwnRadius(incOwnRadius), i_incTargetRadius(incTargetRadius), i_noPlayerTarget(noPlayerTarget) { }
            ...
            bool operator()(Unit* u)
            {
                ...
                return !i_noPlayerTarget || u->GetTypeId() != TYPEID_PLAYER;
            }
        private:
            WorldObject const* i_obj;
            Unit const* i_funit;
